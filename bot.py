...
db_file = "razor.db"
backup_dir = os.path.expanduser("~/razorbot_backups")
    ...
    while True:
        try:
            if os.path.exists(db_file):
                filename = f"razorbot_backup_{datetime.now().strftime('%Y-%m-%d--%H-%M-%S')}.zip"
...
async def money(interaction: discord.Interaction, user: discord.User = None):
    ...
    embed = discord.Embed(title=" RazorMoney Balance", description=f"**{user.mention} has `{bal}` RazorMoney.**", color=0x00ffff)
    await interaction.response.send_message(embed=embed)
...
async def givemoney(interaction: discord.Interaction, user: discord.User, amount: int):
    ...
    conn.commit()
    embed = discord.Embed(description=f"✅ Gave `{amount}` RazorMoney to {user.mention}", color=0x00ff00)
    await interaction.response.send_message(embed=embed)
...
async def topmoney(interaction: discord.Interaction):
    ...
    rows = c.fetchall()
    embed = discord.Embed(title=" Top RazorMoney Users", color=0x00ffff)
